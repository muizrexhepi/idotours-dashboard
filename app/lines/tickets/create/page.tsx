  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/ticket/create/${operator_id}`,
        lineDataForBackend
      );
            ...
            <ul className="space-y-4">
              {selectedStations?.map((station, index) => (
                <li
                  key={index}
                  className={`${
                    activeStation?._id === station._id && "bg-black/10"
                  } flex items-center justify-between space-x-4 p-2 rounded cursor-pointer hover:bg-gray-200 transition-colors`}
                >
                  <div
      ...
      <Card className="w-96">
        <CardHeader>
          <CardTitle className="text-md">
            Linjat e mundshme prej në {activeStation?.city} -{" "}
            {activeStation?.name}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <ul className="space-y-4">
            {activeStation &&
              stationLines[activeStation._id!]?.map((line) => (
                <li key={line._id} className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Input
                      ...
                      placeholder="Price"
                      value={
                        lineData[activeStation._id!]?.[line._id!]?.price || ""
                      }
                      onChange={(e) =>
                        handleInputChange(
                          activeStation._id!,
                    ...
                    <Input
                      ...
                      value={
                        lineData[activeStation._id!]?.[line._id!]
                          ?.childrenPrice || ""
                      }
                      onChange={(e) =>
                        handleInputChange(
                    ...
                    <Input
                      ...
                      value={
                        lineData[activeStation._id!]?.[line._id!]?.time || ""
                      }
                      onChange={(e) =>
                        handleInputChange(
                          activeStation._id!,
                    ...
                    <Input
                      ...
                      value={
                        lineData[activeStation._id!]?.[line._id!]?.duration ||
                        ""
                      }
                      onChange={(e) =>
                        handleInputChange(
